...
재료     materials.html  읽은 것과 이론가와 개념이 서로 어떻게 이어졌나
...
읽기 묶음도 같은 방식으로 이론가와 이어진다. 묶음의 이름과 딸린 줄에서 `말` 을
찾는다. 「가상과 사건 · 마수미」 라고 적어 두면 마수미가 걸린다. 이름에 드러나지
않는 사람은 묶음에 직접 적는다.

```json
"deleuze-practice": {
  "name": "들뢰즈 실천 미학",
  "sub": "『경험론과 주체성』과 로페 주해 · 정독노트 60여 편",
  "이론가": ["deleuze", "hume"],
  "url": "..."
}
```

재료 페이지는 이 셋을 따로 늘어놓지 않는다. 맨 위의 개념 단추 한 줄이 읽기와
이론가를 함께 거른다. 이론가 상자 아래에는 그 사람과 같은 글에 적힌 개념이
붙고, 읽기 묶음 아래에는 그 묶음에서 나온 이론가와 개념이 붙는다. 아직 어느
글에도 쓰지 않은 묶음도 자리를 지킨다. 정독노트가 쌓여 있는데 글이 아직
없다는 사실 자체가 봐야 할 것이기 때문이다.
