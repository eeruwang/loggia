...
function stepsOf(item) {
  ...
  Object.keys(ADD).forEach(function (k) {
    ...
    out.push({ t: a.t, due: a.due, from: a.from, pri: a.pri,
               key: 'add:' + k, addKey: k, fresh: true });
  });
  return out;
}
...
function stepsHtml(item) {
  ...
  function box(st, cls) {
    ...
    // 시작일이 있으면 기간으로 읽는다
    var span = (st.from && st.due) ? '<span class="sspan">' + esc(md(st.from))
             + ' ~ ' + esc(md(st.due)) + '</span>'
