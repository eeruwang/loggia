#!/usr/bin/env node
// =============================================================================
// rawseal.js — 생열쇠 하나로 작은 꾸러미를 봉한다
//
//   node rawseal.js <들어갈 파일> <나올 파일> <열쇠 base64 32바이트>
//
// seal.js 와 무엇이 다른가
//   seal.js 는 사람이 손으로 치는 암호를 받는다. 사람의 암호는 엔트로피가
//   낮으므로 PBKDF2 를 육십만 번 돌려 늘여야 한다. 그 늘이는 일이 무겁다.
//
//   여기서 열쇠를 쥐는 것은 사람이 아니라 워커다. 워커에게는 처음부터
//   무작위 256비트를 주면 된다. 늘일 것이 없다. 그래서 KDF 가 없다.
...
if (!src || !dst || !keyB64) {
  console.error('쓰임  node rawseal.js <들어갈 파일> <나올 파일> <열쇠 base64>');
  process.exit(1);
}

const key = Buffer.from(keyB64, 'base64');
if (key.length !== 32) {
  console.error(`열쇠는 32바이트여야 합니다. 지금은 ${key.length}바이트입니다.`);
  process.exit(1);
}
...
console.log(`  ${dst}  ${Math.round(blob.length / 1024)}KB  (생열쇠로 봉함)`);
