...
#
# 2026년 8월 1일에 판을 브라우저로 옮겼다. 그전에는 파이썬이 그린 다섯 장을
# 각각 잠가 올렸고, 잠근 덩이는 갱신할 때마다 처음부터 끝까지 달라 보이므로
# 글자 하나를 고쳐도 368KB가 저장소에 새로 쌓였다. 지금 갱신 한 번에 바뀌는
# 것은 public/data.enc 하나뿐이다.
#
# 그래서 소금을 다섯 장이 나눠 쓰던 셈이 없어졌다. 소금은 data.enc 안에
# 하나뿐이다. lock.js 도 함께 없어졌다.
#
# 열쇠 넷
#   <암호>        data.enc 를 잠근다. 판을 여는 그 암호다
#   <토큰>        저장소에 올린다
#   DIGEST_KEY    아침 편지 꾸러미를 봉한다. 주지 않으면 그것만 건너뛴다
#   LEDGER_TOKEN  장부 열쇠. 잠기는 데이터 안에 넣는다. 주지 않으면
...
#
# 도구와 워커와 껍데기도 지문에 넣는다. 넣지 않으면 그리는 손을 고쳐도
# 데이터가 그대로일 때 「바뀐 것이 없습니다」라며 고친 손을 올리지 않는다.
# 그러면 저장소의 도구가 조용히 낡고, 다음 사람이 받아 쓰는 것이 옛 도구가 된다.
#
# 글꼴과 data.enc 는 뺀다. 글꼴은 크고 바뀌지 않으며, data.enc 는 잠글 때마다
...
# ── 클라우드플레어가 내주는 자리 ────────────────────────────────────────────
# 워커의 assets 는 public/ 하나만 본다. 껍데기 다섯 장과 옷과 그리는 손과
# 잠긴 데이터가 모두 여기 있다.
...
# 글꼴은 한 번 받으면 바뀌지 않는다. 껍데기와 옷과 손과 알맹이는 갱신될 때마다
...
# 껍데기와 옷과 그리는 손. 자주 바뀌지 않으나 낡으면 판이 어긋난다.
/*.html
...
# 알맹이. 갱신할 때마다 달라진다. 늘 물어보게 한다.
/data.enc
  Cache-Control: no-cache
  X-Content-Type-Options: nosniff
HDR

# 데이터를 잠근다. 장부 열쇠는 이 안에 넣는다.
# 그리는 손은 잠기지 않은 채 저장소에 그대로 있으므로 거기 적을 수 없다.
if [ -f "$DATA" ]; then
  SEALME="$DATA"
  ...
  echo "데이터 파일을 찾지 못했습니다 ($DATA). 껍데기만 올립니다."
fi
...
# 도구가 낡은 채로 남는다. 판은 멀쩡한데 다음 사람이 옛 도구를 받게 된다.
for f in wrangler.jsonc package.json package-lock.json tsconfig.json .gitignore; do
